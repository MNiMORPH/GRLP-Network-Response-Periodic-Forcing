...
out="../../Figures/Figure_S3_SingleSegment_Periods_Water"
...
# ---- CPTs
gmt makecpt -T1.3/2.3/0.2 -Cplasma -D -G0/0.95 -I --COLOR_NAN=white > p.cpt
...
gmt psscale $rgn $proj -Dx1.4i/1.85i+w1i/0.07i+jMC+h -Bx0.2+l"@%2%p@-x,Qw@-     p@-x,Qs@-@%%" -Cp.cpt --MAP_LABEL_OFFSET=3.5p -O -K >> $out.ps
gmt psbasemap $rgn $proj -BnseW -Bx1f3p+l"Period, @%2%P@%% / @%2%T@-eq@-@%%" -By0.2+l"Gain, @%2%G@-z@-@%%" -O -K >> $out.ps

# ---- lag_z
rgn=-R${pmin}/${pmax}/-0.025/0.425
...
awk ' { if ($1==">") print $0 ; else print $1, $3 } ' $basedir/lag_z_num.pg | \
  gmt psxy $rgn $proj -Cp.cpt -W0.8p -O -K >> $out.ps
awk ' { if ($1==">") print $0 ; else print $1, $2 } ' $basedir/lag_z_num.pg | \
  gmt psxy $rgn $proj -Cp.cpt -W0.8p,3_3 -O -K >> $out.ps
awk ' { if ($1==">") print $0 ; else print $1, $4 } ' $basedir/lag_z_num.pg | \
...
# gmt psxy $basedir/lag_Qs_num.pg $rgn $proj -Cp.cpt -W0.8p -O -K >> $out.ps
