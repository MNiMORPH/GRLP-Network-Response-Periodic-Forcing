...
# ---- Variables
out=../../Figures/Figure_S11_SingleSegment_Extra_Examples_Non-Uniform_Width
indir=../../Output/SingleSegment/Figure_S10_S11_SingleSegment_Examples_Non-Uniform_Width
...
for i in ${!periods[@]} ; do
  ...
  proj=-JX2i/0.75i
  gmt psbasemap $rgn $proj -B+n -X2.2i -Y3.7i -O -K >> $out.ps
...
1000 680" | gmt psxy $rgn $proj -Gwhite -O -K >> $out.ps
  ...
  gmt psxy $indir/${periods[$i]}/Qw_ref_profile.te $rgn $proj -W1p,dimgrey,3p_3p -O -K >> $out.ps
  gmt psxy $indir/${periods[$i]}/Qw_ref_profile_tps.te $rgn $proj -Sc3p -Gdimgrey -O -K >> $out.ps
  gmt psxy $indir/${periods[$i]}/Qw_profile.te $rgn $proj -W1p -Cx.cpt -O -K >> $out.ps
  gmt psxy $indir/${periods[$i]}/Qw_profile_tps.te $rgn $proj -Sc3p -Cx.cpt -O -K >> $out.ps
  echo ${z_labels[$i]} | \
    gmt pstext $rgn $proj -F+f11p,Helvetica-Bold,black+jTL+cTL -D0.05i/-0.08i -O -K >> $out.ps
  if [ $i -eq 2 ] ; then
  ...
  gmt psxy $indir/${periods[$i]}/Qw_profile_pert.de $rgn2 $proj2 -W1p -Ct2.cpt -O -K >> $out.ps
  gmt psxy $indir/${periods[$i]}/Qw_profile.de $rgn $proj -W1p -Ct.cpt -O -K >> $out.ps
  echo ${prof_labels[$i]} | \
    gmt pstext $rgn $proj -F+f11p,Helvetica-Bold,black+jTL+cTL -D0.05i/-0.08i -O -K >> $out.ps
  gmt psbasemap $rgn $proj \
    -B${profile_axes[$i]} \
