...
for i in ${!sweepdirs[@]} ; do
  ...
  gmt psxy $basedir/${sweepdirs[$i]}/gain_L.pg $rgn $proj -Sc3p -W0.8p,steelblue -t92 -O -K >> $out.ps
  ...
  gmt psxy $basedir/continuous_gain.pg $rgn $proj -Glightgrey -W0.8p,lightgrey -O -K >> $out.ps
  gmt psxy $basedir/linear_gain.pg $rgn $proj -W0.8p -O -K >> $out.ps
  gmt psxy $basedir/${sweepdirs[$i]}/gain_Le.pg $rgn $proj -Sc3p -W0.8p,steelblue -t92 -O -K >> $out.ps
  ...
  gmt pshistogram $basedir/${sweepdirs[$i]}/Teq.t $rgn_in $proj_in -Gsteelblue -T15+n -O -K >> $out.ps
