...
for i in ${!cases[@]} ; do
  ...
  if [ $i -eq 3 ] ; then
    gmt_extras::plot_key_symbol $rgn $proj 145 140 1.085 "-Sc3p -W0.8p,steelblue" "@%2%N@%%@-1@- = 40" $out
    gmt_extras::plot_key_symbol $rgn $proj 145 140 1.055 "-Sc3p -W0.8p,tomato" "@%2%N@%%@-1@- = [2..150]" $out
    gmt_extras::plot_key_symbol $rgn $proj 145 140 1.025 "-Sd3.5p -W1p,black" "Binned" $out
  fi
