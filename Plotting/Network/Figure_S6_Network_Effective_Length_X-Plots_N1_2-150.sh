...
cross_plot () {
  ...
  xmin=$(cat $basedir/N1_2-150_???.dat | awk ' { print $'$col'} ' | gmt gmtinfo -C | awk ' {print $1 } ')
  xmax=$(cat $basedir/N1_2-150_???.dat | awk ' { print $'$col'} ' | gmt gmtinfo -C | awk ' {print $2 } ')
  ...
  gmt psbasemap $rgn $proj \
    -Bn$axes \
    -Bx$dx+l"$lab" \
    -By60+l"Effective length, @[\widehat{\textit{L}}@[ [km]" \
    -O -K >> $out.ps  
}
...
# ---- Plot Part A
out=../../Figures/Figure_S6a_Network_Effective_Length_X-Plots_N1_2-150
R_B_labs=("a" "g" "m" "s")
R_L_labs=("b" "h" "n" "t")
R_Q_labs=("c" "i" "o" "u")
K_labs=("d" "j" "p" "v")
...
for i in ${!cases[@]} ; do
  ...
  cross_plot ${cases[$i]} 2 "@%2%R@-B@-@%% [-]" 2 "-Y-1.15i -X-5.75i" ${S}eW "${R_B_labs[$i]}"
  cross_plot ${cases[$i]} 3 "@%2%R@-L@-@%% [-]" 2 "-X1.15i" ${S}ew "${R_L_labs[$i]}"
  cross_plot ${cases[$i]} 4 "@%2%R@-Q@-@%% [-]" 2 "-X1.15i" ${S}ew "${R_Q_labs[$i]}"
  cross_plot ${cases[$i]} 5 "@%2%K@%% [-]" 4 "-X1.15i" ${S}ew "${K_labs[$i]}"
  cross_plot ${cases[$i]} 6 "@%2%l@%% [-]" 10 "-X1.15i" ${S}ew "${l_labs[$i]}"
  cross_plot ${cases[$i]} 7 "@~\341@~@%2%l@%%@~\361@~ [-]" 5 "-X1.15i" ${S}ew "${l_mean_labs[$i]}"
...
# ---- Plot Part B
out=../../Figures/Figure_S6b_Network_Effective_Length_X-Plots_N1_2-150
...
for i in ${!cases[@]} ; do
  ...
  cross_plot ${cases[$i]} 8 "@~\341@~@%2%l@-I@-@%%@~\361@~ [-]" 8 "-Y-1.15i -X-5.75i" ${S}eW "${l_mean_head_labs[$i]}"
  cross_plot ${cases[$i]} 9 "@%2%L@%% [km]" 100 "-X1.15i" ${S}ew "${L_labs[$i]}"
  cross_plot ${cases[$i]} 10 "@~\341@~@%2%L@%%@~\361@~ [km]" 40 "-X1.15i" ${S}ew "${L_mean_head_labs[$i]}"
  cross_plot ${cases[$i]} 12 "@%2%w@%% [-]" 10 "-X1.15i" ${S}ew "${w_labs[$i]}"
  cross_plot ${cases[$i]} 13 "@~\341@~@%2%w@%%@~\361@~ [-]" 3 "-X1.15i" ${S}ew "${w_mean_labs[$i]}"
  cross_plot ${cases[$i]} 14 "@%2%p@%% [-]" 0.5 "-X1.15i" ${S}ew "${p_labs[$i]}"
