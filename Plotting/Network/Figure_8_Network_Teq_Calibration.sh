...
for i in ${!sweepdirs[@]} ; do
  ...
  gmt psxy $basedir/MC_N1_40/${sweepdirs[$i]}/gain_L.pg $rgn $proj -Sc3p -W0.8p,steelblue -t90 -O -K >> $out.ps
  ...
  gmt psbasemap $rgn $proj -BnSe${W} -Bx1f3p+l"Period, @%2%P@%% / @%2%T@-eq,max@-@%% [-]" -By0.2+l"Gain, @[\textit{G\textsubscript{Q\textsubscript{s},L}}@[ [-]" -O -K >> $out.ps
  ...
  gmt psxy $basedir/continuous_gain.pg $rgn $proj -Glightgrey -W0.8p,lightgrey -O -K >> $out.ps
  gmt psxy $basedir/linear_gain.pg $rgn $proj -W0.8p -O -K >> $out.ps
  gmt psxy $basedir/MC_N1_40/${sweepdirs[$i]}/gain_Le.pg $rgn $proj -Sc3p -W0.8p,steelblue -t90 -O -K >> $out.ps
