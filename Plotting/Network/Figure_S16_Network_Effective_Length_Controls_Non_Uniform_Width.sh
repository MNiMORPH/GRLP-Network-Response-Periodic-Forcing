...
# ---- Variables
out=../../Figures/Figure_S16_Network_Effective_Length_Controls_Non-Uniform_Width
...
# $k 0" | gmt psxy $rgn $proj -Gtomato -W0.1p,tomato -O -K >> $out.ps
  ...
  awk ' { print $6, $8 } ' $basedir/N1_40_${cases[$i]}.dat | \
  ...
  awk ' { print $6 } ' $basedir/N1_40_${cases[$i]}.dat | \
  ...
  awk ' { print $8 } ' $basedir/N1_40_${cases[$i]}.dat | \
  ...
  gmt psbasemap $rgn_hist2 $proj_hist2 -B+n -X1.2i -O -K >> $out.ps
