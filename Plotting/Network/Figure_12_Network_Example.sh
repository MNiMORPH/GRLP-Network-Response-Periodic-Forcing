...
# ---- Variables
out=../../Figures/Figure_12_Network_Example
...
gmt psscale $rgn $proj -Dx1i/2.15i+w1.5i/0.07i+jMC+h+m -Corder.cpt -B1+l"Stream order, X [-]" \
  --MAP_LABEL_OFFSET=2p \
  --MAP_ANNOT_OFFSET=5p \
  --MAP_TICK_LENGTH=6p \
  --MAP_DEFAULT_PEN=5p \
...
gmt psbasemap $rgn $proj -BnSrW -Bx50+l"Time, @%2%t@%% [kyr]" -By100+l"Elevation [m]" -O -K >> $out.ps
