...
basedir_ref="../../Output/Network/Figure_16_Network_Spatial_Gain_Lag_By_Mean_Length"
out="../../Figures/Figure_S27_Network_Spatial_Gain_Lag_By_Mean_Length_Non-Uniform_Width"
...
for i in ${!nets[@]} ; do
  ...
  gmt psxy $indir_ref/$p/gain.dg $rgn $proj -Wlightgrey -O -K >> $out.ps
  ...
  gmt psxy $indir_ref/$p/lag.dl $rgn $proj -Wlightgrey -O -K >> $out.ps
  ...
  if [ $i -eq 3 ] ; then
    gmt_extras::plot_key_multi_line $rgn $proj 0 12.5 0.085 -W0.8p "Network (non-uniform @%2%B@%%)" $out order.cpt "4 3 2 1"
    gmt_extras::plot_key_line $rgn $proj 0 12.5 0.105 -W0.8p,lightgrey "Network (uniform @%2%B@%%)" $out
