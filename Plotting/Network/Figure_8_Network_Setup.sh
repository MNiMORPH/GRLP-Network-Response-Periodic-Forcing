...
out="../../Figures/Figure_8_Network_Setup"
...
for i in ${!netdirs[@]} ; do
  ...
  gmt psxy $netdir/planform.d $rgn $proj -Corder.cpt -W+cl -O -K >> $out.ps
  gmt psbasemap $rgn $proj -Btslr -Bx20+l"Downstream distance [km]" -O -K >> $out.ps
  echo "(${plan_labs[$i]})" | gmt pstext $rgn $proj -F+f10p,Helvetica-Bold,black+jLT+cLT -D0.05i/-0.08i -O -K >> $out.ps
  ...
  rgn=-R-15/115/$qmin/$qmax
  proj=-JX1.5i
  gmt psbasemap $rgn $proj -B+n -Y-1.65i -O -K >> $out.ps
  gmt psxy $netdir/discharge.dq $rgn $proj -W+cl -Corder.cpt -O -K >> $out.ps
  echo "(${discharge_labs[$i]})" | gmt pstext $rgn $proj -F+f10p,Helvetica-Bold,black+jRB+cRB -D-0.04i/0.06i -O -K >> $out.ps
  ...
  gmt psxy $netdir/profile.de $rgn $proj -W+cl -Corder.cpt -O -K >> $out.ps  
  echo "(${prof_labs[$i]})" | gmt pstext $rgn $proj -F+f10p,Helvetica-Bold,black+jLB+cLB -D0.05i/0.08i -O -K >> $out.ps
  if [ $i -eq 3 ] ; then
    gmt psscale $rgn $proj \
      ...
      --FONT=16p \
      -O -K >> $out.ps
    gmt_extras::plot_discharge_key $rgn $proj 108 325 450 $out
  fi
  gmt psbasemap $rgn $proj \
    -BnSe${W} \
    -Bx20+l"Downstream distance, @%2%x@%% [km]" \
  ...
  echo "(${ratio_labs[$i]})" | gmt pstext $rgn $proj -F+f10p,Helvetica-Bold,black+jLB+cLB -D0.05i/0.08i -O -K >> $out.ps
  gmt psbasemap $rgn $proj \
    -BtS \
