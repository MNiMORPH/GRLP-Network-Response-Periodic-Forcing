...
# variables
out="../../Figures/Figure_S24_Network_z_Q_Gain_Lag_Non-Uniform_Width_N1_40"
proj=-JX1.5il/1.5i
...
for i in ${!sweepdirs[@]} ; do
  ...
  gmt psxy $basedir/MC_N1_40/${sweepdirs[$i]}/z_Q_gain.pg $rgn $proj -Ey+a+p0.6p,steelblue+w2p -t95 -O -K >> $out.ps
  gmt psxy $basedir/MC_N1_40/${sweepdirs[$i]}/z_Q_gain.pg $rgn $proj -Sc3.5p -W0.6p,steelblue -t75 -O -K >> $out.ps
  ...
  gmt psxy $basedir/MC_N1_40/${sweepdirs[$i]}/z_Q_lag.pl $rgn $proj -Ey+a+p0.6p,steelblue+w2p -t95 -O -K >> $out.ps
  gmt psxy $basedir/MC_N1_40/${sweepdirs[$i]}/z_Q_lag.pl $rgn $proj -Sc3.5p -W0.6p,steelblue -t75 -O -K >> $out.ps
  echo "(${lag_z_labels[$i]})" | gmt pstext $rgn $proj -F+f10p,Helvetica-Bold,black+jTL+cTL -D0.04i/-0.06i -O -K >> $out.ps
  gmt psbasemap $rgn_l $proj -BnS${E}${W} -Bx1f3p+l"Period, \textit{P} / @[\widehat{\textit{T\textsubscript{eq}}}@[ [-]" -By0.2+l"Lag, @~\152@~@%2%@-z@-@%% / @%2%P@%% [-]" -O -K >> $out.ps
...
0.12 2.4
0.12 0.75
0.05 0.75" | gmt psxy $rgn $proj -W0.8p -O -K >> $out.ps
echo "0.12 1.575 @~\144@~@%2%z@%%: Varying water supply" | gmt pstext $rgn $proj -F+f8p+a270 -Gwhite -N -O -K >> $out.ps
