...
# ---- Variables
out="../../Figures/Figure_S21_Network_Effective_Length_N1_Non-Uniform_Width"
rgn=-R-5/155/1/1.6
proj=-JX1.5i
basedir="../../Output/Network/Figure_S21_Network_Effective_Length_N1_Non-Uniform_Width"
...
for i in ${!cases[@]} ; do
  ...
  if [ $i -eq 3 ] ; then
    gmt_extras::plot_key_symbol $rgn $proj 145 140 1 "-Sc3p -W0.8p,steelblue" "@%2%N@%%@-1@- = 40" $out
    gmt_extras::plot_key_symbol $rgn $proj 145 140 0.965 "-Sc3p -W0.8p,tomato" "@%2%N@%%@-1@- = [2..150]" $out
    gmt_extras::plot_key_symbol $rgn $proj 145 140 0.93 "-Sd3.5p -W1p,black" "Binned" $out
  fi
