...
if output_gmt:
    ...
    for i,case in enumerate(nets[0].keys()):
        ...
        with open(basedir + case + "/gain_L.pg", "wb") as f:
            arr = np.column_stack(( 
                [p/lp.equilibration_time for gs in gains for p in gs[case]['P']],
                [g[0][-1] for gs in gains for g in gs[case]['G_Qs']['Qs']]
                ))
        ...
        with open(basedir + case + "/gain_Le.pg", "wb") as f:
            arr = np.column_stack(( 
                [p/Teqs[i] for i,gs in enumerate(gains) for p in gs[case]['P']],
                [g[0][-1] for gs in gains for g in gs[case]['G_Qs']['Qs']]
                ))
        ...
        with open(basedir + case + "/Teq.t", "wb") as f:
            arr = [gs[case]['Teq']/3.15e10 for gs in gains]
            np.savetxt(f, arr)