"""
This script performs the analysis presented in Figure 12 of McNab et al. (2025,
...
for i,seg in enumerate(nets[neti]['UUU'].list_of_LongProfile_objects):
    ...
    plt.text(seg.x.mean()/1.e3, lags[neti]['UUU']['lag_z']['Qs'][Pi][i].mean(), i)
plt.show()
...
# trunk_segs = nets[neti]['UUU'].find_downstream_IDs(34)
# trunk_x_stack = np.hstack(( [nets[neti]['UUU'].list_of_LongProfile_objects[i].x[:-1]/1.e3 for i in trunk_segs] ))
# trunk_y_stack = np.hstack(( [plan[i]['y'][:-3] for i in trunk_segs] ))
# trunk_z_stack = np.hstack(( [periodic['z'][i][:,:-1] for i in trunk_segs] ))
# nodes_to_plot = np.arange(0, len(trunk_x_stack)-1, 12).astype(int)
...
for j,segs in enumerate(segs_to_plot):
    ...
    x_stack = np.hstack(( [nets[neti]['UUU'].list_of_LongProfile_objects[i].x[:-1]/1.e3 for i in segs] ))
    y_stack = np.hstack(( [plan[i]['y'][:-3] for i in segs] ))
    z_stack = np.hstack(( [periodic['z'][i][:,:-1] for i in segs] ))
    nodes_to_plot = np.hstack(( np.arange(0, len(x_stack)-1, 12), len(x_stack)-2)).astype(int)    
    
    axs[0,0].plot(x_stack[nodes_to_plot], y_stack[nodes_to_plot], "o", ms=4, mec='k', mfc='none')
...
if output_gmt:
    ...
    basedir = "../../Output/Network/Figure_12_Network_Example/"
