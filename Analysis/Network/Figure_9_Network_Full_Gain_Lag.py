...
if output_gmt:
    ...
    for case in ['UUU', 'NUU', 'UAU', 'NAU']:
        ...
        with open(basedir + case + "/z_gain.pg", "wb") as f:
            arr = np.column_stack((
                [p/Teqs[i] for i,gs in enumerate(gains) for p in gs[case]['P']],
                [g[0][-1] for gs in gains for g in gs[case]['G_z']['Qs']],
                [g[0][-1]-np.hstack(g).min() for gs in gains for g in gs[case]['G_z']['Qs']],
                [np.hstack(g).max()-g[0][-1] for gs in gains for g in gs[case]['G_z']['Qs']]
                ))
            np.savetxt(f, arr)
        
        with open(basedir + case + "/z_lag.pl", "wb") as f:
            arr = np.column_stack((
                [p/Teqs[i] for i,gs in enumerate(lags) for p in gs[case]['P']],
                [l[0][-1] for ls in lags for l in ls[case]['lag_z']['Qs']],
                [l[0][-1]-np.hstack(l).min() for ls in lags for l in ls[case]['lag_z']['Qs']],
                [np.hstack(l).max()-l[0][-1] for ls in lags for l in ls[case]['lag_z']['Qs']]
                ))
            np.savetxt(f, arr)
            
        with open(basedir + case + "/z_Q_gain.pg", "wb") as f:
            arr = np.column_stack((
                [p/Teqs[i] for i,gs in enumerate(gains) for p in gs[case]['P']],
                [g[0][-1] for gs in gains for g in gs[case]['G_z']['Qw']],
                [g[0][-1]-np.hstack(g).min() for gs in gains for g in gs[case]['G_z']['Qw']],
                [np.hstack(g).max()-g[0][-1] for gs in gains for g in gs[case]['G_z']['Qw']]
                ))
            np.savetxt(f, arr)
        
        with open(basedir + case + "/z_Q_lag.pl", "wb") as f:
            arr = np.column_stack((
                [p/Teqs[i] for i,gs in enumerate(lags) for p in gs[case]['P']],
                [l[0][-1] for ls in lags for l in ls[case]['lag_z']['Qw']],
                [l[0][-1]-np.hstack(l).min() for ls in lags for l in ls[case]['lag_z']['Qw']],
                [np.hstack(l).max()-l[0][-1] for ls in lags for l in ls[case]['lag_z']['Qw']]
                ))
            np.savetxt(f, arr)

            
        with open(basedir + case + "/Qs_gain.pg", "wb") as f:
            arr = np.column_stack((
                [p/Teqs[i] for i,gs in enumerate(gains) for p in gs[case]['P']],
                [g[0][-1] for gs in gains for g in gs[case]['G_Qs']['Qs']]
                ))
        ...
        with open(basedir + case + "/Qs_lag.pl", "wb") as f:
            arr = np.column_stack((
                [p/Teqs[i] for i,ls in enumerate(lags) for p in ls[case]['P']],
                [l for ls in lags for l in ls[case]['lag_Qs']['Qs']]
                ))
        ...
        with open(basedir + case + "/Qs_Qw_gain.pg", "wb") as f:
            arr = np.column_stack((
                [p/Teqs[i] for i,gs in enumerate(gains) for p in gs[case]['P']],
                [g[0][-1] for gs in gains for g in gs[case]['G_Qs']['Qw']]
                ))
        ...
        with open(basedir + case + "/Qs_Qw_lag.pl", "wb") as f:
            arr = np.column_stack((
                [p/Teqs[i] for i,ls in enumerate(lags) for p in ls[case]['P']],
                [l for ls in lags for l in ls[case]['lag_Qs']['Qw']]
                ))
