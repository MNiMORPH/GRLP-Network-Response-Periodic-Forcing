...
mean_lengths = [net['UUN'].mean_head_length for net in nets]
...
for i in [0, 66, 132, 199]:
    ...
    # Predict network Teq
    Teq = sorted_gains[i]['UUN']['Teq']
    L_eff = np.sqrt(Teq*sorted_nets[i]['UUN'].mean_diffusivity)

    # Evolve to ensure steady state
    sorted_nets[i]['UUN'].evolve_threshold_width_river_network(nt=100, dt=3.154e10)
    
    # Evolve with sinusoisal variation in sediment supply and period equal to
    # equilibration time.
    periodic = grlpx.evolve_network_periodic(
        net=copy.deepcopy(sorted_nets[i]['UUN']),
        period=Teq,
        A_Qs=0.2,
        A_Q=0.
        )
...
for i in [0, 66, 132, 199]:
    ...
    single_segs_A.append(grlpx.generate_single_segment_network(
        L=sorted_nets[i]['UUN'].list_of_LongProfile_objects[0].x_ext[0][-1],
        Q_mean=Q_mean,
        Qs_mean=Qs_mean,
        B_mean=B_mean,
        p_Q=sorted_hacks[i]['UUN']['p'],
        p_B=0,
        x0=x0,
        dx=5.e2,
        evolve=True
...
for i,neti in enumerate([0, 66, 132, 199]):
    ...
    # Predict network Teq
    Teq = sorted_gains[neti]['UUN']['Teq']
    L_eff = np.sqrt(Teq*sorted_nets[neti]['UUN'].mean_diffusivity)
    L = sorted_nets[neti]['UUN'].list_of_LongProfile_objects[0].x.max()
    ...
    for j,seg in enumerate(sorted_nets[neti]['UUN'].list_of_LongProfile_objects):
