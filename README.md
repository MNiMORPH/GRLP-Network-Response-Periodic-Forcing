# Software and data supplement to "Effects of network geometry on aggradation, incision and sediment transport by alluvial rivers" by M<sup>c</sup>Nab et al. (2025, EGUsphere)

This repository contains scripts and data to reproduce the analyses and figures presented in:

- M<sup>c</sup>Nab, F., Schildgen, T.F., Turowski, J.M. and Wickert, A.D., 2025, Effects of network geometry on aggradation, incision and sediment transport by alluvial rivers, *EGUSphere [Preprint]*, doi.
...
- `Analysis/`: Directory containing `Python` scripts for performing the core analysis of the paper.
  ...
  - `Network/`: Scripts associated with network analysis.
    ...
    - `Expected_Length/`: `C++` script and `python` wrapper to compute expected network lengths.
...
The scripts in `Plotting/` use `Bash` shell, which will be available by default on most `UNIX` systems. They primarily use commands from the `Generic Mapping Tools v6.5` (Wessel et al., 2019). See the documentation for instructions on how to install GMT. They also use `ImageMagick`'s convert command to convert `GMT`'s native post-script files to PNG. Once you have GMT installed, you should be able to run the scripts in the Plotting/ directory.
...
- M<sup>c</sup>Nab, F., Schildgen, T.F., Turowski, J.M. and Wickert, A.D., 2025, Effects of network geometry on aggradation, incision and sediment transport by alluvial rivers, *EGUSphere [Preprint]*, doi.
- M<sup>c</sup>Nab, F., 2025, Software and data supplement to "Effects of network geometry on aggradation, incision and sediment transport by alluvial rivers" by M<sup>c</sup>Nab et al. (2025, EGUsphere), Version 1, *Zenodo*, doi.
...
- Wickert, A.D., M<sup>c</sup>Nab, F. and Barefoot, E., 2024, GRLP, Version 2.0.0-beta, *Zenodo*, doi.